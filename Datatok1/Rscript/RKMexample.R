...
outRKM = cluspca(Data[,c(-1,-2)], 3, 2, method = "RKM", rotation = "varimax", scale = FALSE, nstart = 10) 
summary(outRKM)
plot(outRKM, cludesc = TRUE)


selPar(data=Data[,c(-1,-2)],mnclus=5,mndim=5,ndiv=150,R=30)
outRKM2 = cluspca(Data[,c(-1,-2)], 2, 2, method = "RKM", rotation = "varimax", scale = FALSE, nstart = 10) 
summary(outRKM2)
plot(outRKM2, cludesc = TRUE)
...
for(i in 1:length(Data[,1]))
{
  ...
  {
    temp1 = as.numeric(Data[i,1]==Data[ii,1])
    temp2 = as.numeric(outRKM2$cluster[i]==outRKM2$cluster[ii])
    temp3 = temp3 + as.numeric(temp1+temp2==1)
  }
}
